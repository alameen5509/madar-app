...
import { calcGoalProgress, type RoleDim, type RoleGoalData } from "@/components/RoleTree";
import { NewTaskDialog } from "@/app/tasks/page";

interface ProjectInfo { id: string; title: string; description?: string; status?: string; targetDate?: string; progressPercent?: number; }
interface TaskInfo { id: string; title: string; status: string; description?: string; dueDate?: string; userPriority?: number; }

export default function RoleGoalPage({ params }: { params: Promise<{ slug: string; goalId: string }> }) {
  ...
  useEffect(() => {
    if (!goal) return;
    setProjects(goal.projects.map(pid => allProjects.find(p => p.id === pid) ?? { id: pid, title: "...", status: "Active" }));
    setTasks(goal.tasks.map(tid => allTasks.find(t => t.id === tid) ?? { id: tid, title: "...", status: "Pending" }));
  }, [goal, allProjects, allTasks]);

  const dimChain: RoleDim[] = [];
  const goalChain: RoleGoalData[] = [];
  if (goal) {
    ...
    for (let cg: RoleGoalData | undefined = goal; cg; cg = cg.parentGoalId ? goals.find(g => g.id === cg!.parentGoalId) : undefined) {
  ...
  async function addSubGoal() {
    ...
    await api.post(`/api/role-goals/${goalId}/subgoal`, { title: fTitle.trim(), description: fDesc.trim() || undefined }).catch(() => {});
    resetForm(); load();
  }
  ...
  async function createProject() {
    ...
    try {
      ...
      if (data?.id) {
        await api.post(`/api/role-goals/${goalId}/link-project`, { id: data.id });
      }
  ...
  async function createTask() {
    ...
    try {
      ...
      await api.post(`/api/role-goals/${goalId}/link-task`, { id: data.id });
    } catch {}
    resetForm(); load();
  }

  async function linkProject(pid: string) { await api.post(`/api/role-goals/${goalId}/link-project`, { id: pid }).catch(() => {}); setShowAdd(null); load(); }
  async function linkTask(tid: string) { await api.post(`/api/role-goals/${goalId}/link-task`, { id: tid }).catch(() => {}); setShowAdd(null); load(); }
  async function unlinkProject(pid: string) { await api.delete(`/api/role-goals/${goalId}/projects/${pid}`).catch(() => {}); load(); }
  async function unlinkTask(tid: string) { await api.delete(`/api/role-goals/${goalId}/tasks/${tid}`).catch(() => {}); load(); }
  ...
  async function updateProgress(p: number) { await api.put(`/api/role-goals/${goalId}`, { progress: p }).catch(() => {}); load(); }
  ...
  const gc = progress >= 100 ? "#3D8C5A" : progress >= 50 ? "#5E5495" : "#D4AF37";
    ...
    <main className="flex-1 overflow-y-auto" style={{ background: "var(--bg)" }}>
        ...
        <div className="flex items-center gap-1 text-[10px] mb-2 flex-wrap">
          ...
          {dimChain.map(d => (
            <span key={d.id} className="flex items-center gap-1">
              <span style={{ color: "var(--muted)" }}>←</span>
              <Link href={`/circles/${slug}/dimensions/${d.id}`} className="hover:underline" style={{ color: "var(--muted)" }}>{d.name}</Link>
            </span>
          ))}
          {goalChain.map((g, i) => (
            <span key={g.id} className="flex items-center gap-1">
              ...
              {i < goalChain.length - 1
                ? <Link href={`/circles/${slug}/goals/${g.id}`} className="hover:underline" style={{ color: "var(--muted)" }}>{g.title}</Link>
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setShowAdd(showAdd === "newproject" ? null : "newproject")} className="px-3 py-2 rounded-xl text-[10px] font-bold text-white" style={{ background: "#5E5495" }}>+ مشروع جديد</button>
          <button onClick={() => setShowAdd(showAdd === "linkproject" ? null : "linkproject")} className="px-3 py-2 rounded-xl text-[10px] font-bold border" style={{ borderColor: "#5E5495", color: "#5E5495" }}>ربط مشروع</button>
        ...
        {showAdd === "newproject" && (
          <div className="p-5 rounded-2xl border-2 fade-up space-y-3" style={{ background: "var(--card)", borderColor: "#5E549540" }}>
            <p className="text-xs font-bold" style={{ color: "#5E5495" }}>مشروع جديد</p>
            ...
            <div className="flex gap-2"><button onClick={createProject} className="px-4 py-2 rounded-xl text-xs font-bold text-white" style={{ background: "#5E5495" }}>إنشاء وربط</button><button onClick={resetForm} className="px-3 py-2 rounded-xl text-xs text-[#6B7280] bg-gray-100">إلغاء</button></div>
          </div>
        )}
        {showAdd === "linkproject" && (
          <div className="p-5 rounded-2xl border-2 fade-up space-y-2" style={{ background: "var(--card)", borderColor: "#5E549540" }}>
            <p className="text-xs font-bold" style={{ color: "#5E5495" }}>ربط مشروع موجود</p>
            <div className="max-h-48 overflow-y-auto space-y-1">
              {allProjects.filter(p => !goal.projects.includes(p.id)).map(p => (
                <button key={p.id} onClick={() => linkProject(p.id)} className="w-full text-right px-3 py-2.5 rounded-xl text-xs hover:bg-purple-50 transition border" style={{ borderColor: "var(--card-border)", color: "var(--text)" }}>📦 {p.title}</button>
              ))}
              {allProjects.filter(p => !goal.projects.includes(p.id)).length === 0 && <p className="text-[10px] text-center py-3" style={{ color: "var(--muted)" }}>لا توجد مشاريع</p>}
            </div>
        ...
        {showAdd === "newtask" && (
          <NewTaskDialog goals={[]} onClose={() => setShowAdd(null)}
            onCreated={async (t) => {
              try { await api.post(`/api/role-goals/${goalId}/link-task`, { id: t.id }); } catch {}
              load();
            }} />
        )}
        {showAdd === "linktask" && (
          <div className="p-5 rounded-2xl border-2 fade-up space-y-2" style={{ background: "var(--card)", borderColor: "#3D8C5A40" }}>
            ...
            <div className="max-h-48 overflow-y-auto space-y-1">
              ...
              {allTasks.filter(t => t.status !== "Completed" && !goal.tasks.includes(t.id)).length === 0 && <p className="text-[10px] text-center py-3" style={{ color: "var(--muted)" }}>لا توجد مهام</p>}
            </div>
            <button onClick={resetForm} className="text-xs text-[#6B7280]">إلغاء</button>
          </div>
        )}
        ...
        {subGoals.length > 0 && (
          <div>
            ...
            <div className="space-y-3">{subGoals.map(sg => {
              ...
              const sgc = sgp >= 100 ? "#3D8C5A" : sgp >= 50 ? "#5E5495" : "#D4AF37";
              return (
                <Link key={sg.id} href={`/circles/${slug}/goals/${sg.id}`} className="group block rounded-2xl p-4 border transition-all hover:shadow-md" style={{ background: "var(--card)", borderColor: "var(--card-border)" }}>
              ...
              <div key={p.id} className="rounded-2xl border overflow-hidden" style={{ background: "var(--card)", borderColor: "var(--card-border)" }}>
                {editingProject === p.id ? (
                  <div className="p-4 space-y-2">
                    <div className="flex items-center justify-between mb-1"><p className="text-xs font-bold" style={{ color: "#5E5495" }}>تعديل المشروع</p>
                      <div className="flex gap-2"><button onClick={() => setEditingProject(null)} className="px-3 py-1 rounded-lg text-[10px] text-[#6B7280] bg-gray-100">إلغاء</button><button onClick={saveEditProject} className="px-3 py-1 rounded-lg text-[10px] font-bold text-white" style={{ background: "#5E5495" }}>حفظ</button></div>
                    </div>
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        <Link href={`/projects?id=${p.id}`} className="text-[9px] px-2 py-1.5 rounded-lg font-bold hover:bg-purple-50 transition" style={{ color: "#5E5495" }}>فتح ←</Link>
                        <button onClick={() => { setEditingProject(p.id); setEpTitle(p.title); setEpDesc(p.description ?? ""); setEpDate(p.targetDate?.slice(0, 10) ?? ""); }}
                          className="text-[9px] px-2 py-1 rounded-lg text-[#5E5495] hover:bg-purple-50 transition">تعديل</button>
